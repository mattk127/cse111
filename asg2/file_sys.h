...
class inode {
   ...
   public:
      ...
      void setContents(base_file_ptr);
...
private:
  ...
  public:
    ...
    virtual const wordvec& readF() const = 0;
    virtual void writeF (const wordvec& newdata) = 0;
...
class plain_file: public base_file {
   ...
   public:
      ...
      virtual const wordvec& readF() const override;
      virtual void writeF (const wordvec& newdata) override;
...
class directory: public base_file {
  ...
  public:
   ...
   virtual inode_ptr mkdir (const string& dirname) override;
   virtual inode_ptr mkfile (const string& filename) override;
 virtual void createF(const string& filename,const wordvec& newdata);
   virtual void createDir (const string& filename);
   virtual inode_ptr getF (const string& filename);
   virtual void addent (const string& filename, const inode_ptr& file);
   virtual string printEnt();
   virtual string printEntR(string content,vector<int> *visited);
   virtual int numF();
   virtual void clearDir();
   virtual bool fexists(const string& filename);
