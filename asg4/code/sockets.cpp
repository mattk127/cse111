...
void base_socket::close() {
   ...
   if (status < 0) throw socket_sys_error 
   ("close("+ to_string(socket_fd) + ")");
   socket_fd = CLOSED_FD;
}
...
void base_socket::accept (base_socket& socket) const {
  ...
  if (socket.socket_fd < 0) throw socket_sys_error
  ("accept");
}

ssize_t base_socket::send (const void* buffer, size_t bufsize) {
  int nbytes = ::send (socket_fd, buffer,
  bufsize, MSG_NOSIGNAL);
...
void base_socket::connect (const string host, const in_port_t port) {
  ...
  if (hostp == NULL) throw socket_h_error 
  ("gethostbyname(" + host + ")");
  socket_addr.sin_family = AF_INET;
  socket_addr.sin_port = htons (port);
  socket_addr.sin_addr = *reinterpret_cast<in_addr*> (hostp->h_addr);
  int status = ::connect (socket_fd, 
  reinterpret_cast<sockaddr*> (&socket_addr), sizeof (socket_addr));
  if (status < 0) throw socket_sys_error 
...
void base_socket::set_socket_fd (int fd) {
  ...
  if (rc < 0) throw socket_sys_error 
  ("set_socket_fd(" + to_string (fd) + "): getpeername");
  socket_fd = fd;
...
string to_string (const in_addr& ipv4_addr) { 
  ...
  (AF_INET, &ipv4_addr, buffer, sizeof buffer);
...
string to_string (const base_socket& sock) {
  ...
  + to_string (info.addresses[0]) + ") port " + to_string 
  (ntohs (sock.socket_addr.sin_port));
}
...
hostinfo::hostinfo (const in_addr& ipv4_addr):
  ...
  (&ipv4_addr, sizeof ipv4_addr,AF_INET)) {
}
